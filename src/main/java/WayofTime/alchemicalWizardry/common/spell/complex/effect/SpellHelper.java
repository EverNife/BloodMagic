    ...
    public static void smeltBlockInWorld(World world, int posX, int posY, int posZ) {
        ...
        if (smeltedStack != null && smeltedStack.getItem() instanceof ItemBlock) {
            world.setBlock(
                    ...
                    posY,
                    posZ,
                    ((ItemBlock) (smeltedStack.getItem())).field_150939_a,
                    smeltedStack.getItemDamage(),
                    3);
        }
    }
        ...
        switch (index) {
            case 1:
                SpellHelper.sendParticleToPlayer(
                        player,
                        "mobSpell",
                        xCoord + 0.5D + rand.nextGaussian() / 8,
                        yCoord + 1.1D,
                        zCoord + 0.5D + rand.nextGaussian() / 8,
                        0.5117D,
                        0.0117D,
                        0.0117D);
                break;
            ...
            case 4:
                ...
                for (int l = 0; l < 8; ++l) {
                    SpellHelper.sendParticleToPlayer(
                            player,
                            "reddust",
                            xCoord + Math.random() - Math.random(),
                            yCoord + Math.random() - Math.random(),
                            zCoord + Math.random() - Math.random(),
                            f1,
                            f2,
                            f3);
                }
        ...
        if (inventory instanceof ISidedInventory) {
            ...
            for (int in : array) {
                canBeInserted[in] = inventory.isItemValidForSlot(in, stack)
                        && ((ISidedInventory) inventory).canInsertItem(in, stack, dir.ordinal());
            }
        } else {
